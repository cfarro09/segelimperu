...
// JS REGISTER START*************************************************************
		...
		document.querySelector('#form_add_detail').addEventListener('submit', (e) => {
			...
			if (edit.value && parseInt(edit.value) > 0) {
				...
				if ($('#thispage').length > 0 && $('#thispage').val() == "edit") {
					fetch_post(`${site_url}/VoletasUnif/edit_detalle_id`, $('#form_add_detail').serialize()).then(res => {
						if (res) {
							if (res.success) {
				...
				index++;
				if ($('#thispage').length > 0 && $('#thispage').val() == "edit") {
					fetch_post(`${site_url}VoletasUnif/edit_detalle_insert`, $('#form_add_detail').serialize()+`&nro_voleta=${$('#nro_voleta').val()}`).then(res => {
						if (res) {
							if (res.success) {
								swal('Bien!', res.msg, 'success' );
							}else{
	...
	$("#columns_table").on ("click", ".delete_row", function (e) {
		...
		if ($(this).data('id_detalle')) {
			...
			fetch_post(`${site_url}/VoletasUnif/edit_vol_del_detalle`, `id_detalle=${id_detalle}`).then(res => {
				if (res) {
					if (res.success) {
						swal('Bien!', res.msg, 'success' );
						$('#columns_table').empty();
	...
	$('#form_insert_voleta').on('submit', e => {
		e.preventDefault();
		if ($('#columns_table tr').length > 0 || $('#estado_boleta').val() == "anulado") {
			...
			let to_send = "";
			if ($('#estado_boleta').val() == "anulado") {
				to_send = $('#form_insert_voleta').serialize();
			}else{
				document.querySelectorAll('#columns_table tr').forEach(tr => {
					...
					data[index]['nro_voleta'] = $('#nro_voleta').val()
					index++;
				})
				to_send = $('#form_insert_voleta').serialize()+"&data_extra="+JSON.stringify(data);
			}
			fetch_post(`${site_url}/VoletasUnif/register_voleta`, to_send).then(res => {
				if (res) {
					if (res.success) {
						swal('Bien!', res.msg, 'success' );
						$('#columns_table').empty();
						document.querySelector('#form_insert_voleta').reset();
					}else{
						swal('Error!', 'Hubo un error, vuelva a intentarlo', 'error')
					}
				}else{
			...
			swal('Error!', 'El detalle de la voleta esta vacia', 'error')
		}
	})
// JS REGISTER END*************************************************************	
...
// JS LISTAR END*****************************************************************
// JS EDIT START*****************************************************************
	$('#form_edit_voleta').on('submit', e => {
		e.preventDefault();
		fetch_post(`${site_url}/VoletasUnif/edit_vol_nro_vol`, $('#form_edit_voleta').serialize()).then(res => {
			if (res) {
				if (res.success) {
					swal('Bien!', res.msg, 'success' );
					$('#columns_table').empty();
